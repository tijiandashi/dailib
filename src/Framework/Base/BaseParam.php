...
class BaseParam
{
    ...
    public function vaild(& $classIns, $di, $basePageInfo)
    {
        ...
        foreach ( $reflection->getProperties() as $property) {
            ...
            $name = \Dai\Lib\Framework\Plugin\Annotations::getCommentValue($comment, 'name' );
            $name = $name == "" ? $propertyName : $name;
            $value =  ($basePageInfo->requestType == "post") ? $request->getPost($name) : $request->getQuery($name);

            $type = \Dai\Lib\Framework\Plugin\Annotations::getCommentValue($comment, 'type' );
            $length = \Dai\Lib\Framework\Plugin\Annotations::getCommentValue($comment, 'length' );
            $regex = \Dai\Lib\Framework\Plugin\Annotations::getCommentValue($comment, 'regex' );
            $optional = \Dai\Lib\Framework\Plugin\Annotations::getCommentValue($comment, 'optional' );
